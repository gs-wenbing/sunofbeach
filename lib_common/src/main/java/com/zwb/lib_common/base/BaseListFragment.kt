...
abstract class BaseListFragment<T, VB : ViewBinding, VM : BaseViewModel> : BaseFragment<VB, VM>() {
    ...
    fun init(
        ...
        mRefreshLy.setRefreshHeader(ClassicsHeader(requireContext()))
        mRecyclerListener = recyclerListener
        initListener()
        setScroll(mCommRecycler)
    ...
    /**
     * 这个key 一般是请求数据的url
     * 作用是请求返回（或异常）后处理LoadSir的Callback, 同样的请求数据时也要把这个key传过去  （有点烦）
     */
    ...
    fun loadCompleted(action: Int, list: List<T>? = null, pageSize: Int = mPageSize, msg: String? = "") {
...
//            }
        ...
        if(action == ACTION_DEFAULT || action == ACTION_REFRESH){
            if (mAdapter.itemCount == 0) {
                mViewModel.loadState.value = State(StateType.EMPTY, loadKey)
            } else {
                mViewModel.loadState.value = State(StateType.SUCCESS, loadKey)
            }
        }
    }
